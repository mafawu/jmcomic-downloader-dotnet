...
- 本地模式：管理多个本地目录，离线浏览已下载漫画（封面 / 名字 / 标签），点击打开所在文件夹，卡片分格可调
- 分格调节：本地与在线（搜索 / 分类 / 排行 / 周榜 / 收藏，多数据源）统一卡片大小滑块，随窗口宽度自动缩放
...
> 注意：配置文件中的用户名、密码和标题翻译 API key 在 Windows 上使用 DPAPI（当前用户作用域）加密存储；旧版明文配置会在首次启动时自动迁移为加密格式。
>
